...
/**
 * notification-service consumes all four platform topics
 * (user-events, order-events, payment-events, inventory-events) — it
 * never produces any event itself, so this config only needs consumer
 * plumbing (still using a KafkaTemplate for the dead-letter publisher).
 */
@Configuration
@EnableKafka
public class KafkaConsumerConfig {
    ...
    public ConsumerFactory<String, Object> consumerFactory() {
        ...
        props.put(JsonDeserializer.TRUSTED_PACKAGES, "com.ecommerce.notificationservice.kafka.event");
    ...
    private DefaultErrorHandler errorHandler(KafkaOperations<String, Object> kafkaTemplate) {
    ...
    public NewTopic userEventsDlt() {
        return TopicBuilder.name("user-events.DLT").partitions(3).replicas(1).build();
    }
